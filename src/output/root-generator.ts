...
export function generateRoot(stats: RepoStats, conventions?: ConventionsData): string {
  ...
  lines.push(`- **cartograph_table_usage** \`<table>\` — bridge database tables to mapped entities, mapped columns, entity touchpoints, direct table-name references, and upstream framework wiring through table-backed adapters`);
  lines.push(`- **cartograph_test_targets** — suggest likely test files for a symbol, file, or table (heuristic, ranked, not exhaustive)`);
  lines.push(`- **cartograph_scaffold_plan** \`<reference> <target>\` — plan the files and class names needed to mirror an existing slice for a new target; does not write files`);
