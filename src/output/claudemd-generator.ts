...
export function generateClaudeMdSection(stats: RepoStats, conventions: ConventionsData): string {
  ...
  lines.push(`5. Use \`cartograph_table_usage\` when you need the schema↔code bridge: mapped entities, mapped columns, entity-based touchpoints, direct table-name references, and upstream framework wiring for a table`);
  ...
  lines.push('| `cartograph_table_usage` | Bridge schema to code: show Doctrine-style mapped entities, mapped columns, entity-based touchpoints, indexed direct table-name references, and upstream framework wiring through table-backed adapters such as Models, Repositories, Builders, and DataObjects. Best for table→API tracing. It may still miss patterns with no explicit entity mapping, symbol reference, or table-name signal. | `name`: table name. `depth`: entity-graph depth. `limit`: max touchpoints. `includeTests`: include tests |');
