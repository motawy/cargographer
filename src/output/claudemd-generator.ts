...
export function generateClaudeMdSection(stats: RepoStats, conventions: ConventionsData): string {
  ...
  lines.push(`7. Use \`cartograph_sql_validate\` for review/debug questions like "does this Builder SQL match the schema?" — it validates literal SQL-ish table, column, and join refs inside a symbol or file against the current indexed schema`);
  lines.push(`8. Use \`cartograph_test_targets\` after identifying the code you will touch — provide exactly one of \`symbol\`, \`file\`, or \`table\`; it uses direct test-side imports / instantiations when available, but still returns ranked suggestions rather than an exhaustive list`);
  lines.push(`9. Use \`cartograph_scaffold_plan\` when cloning an existing endpoint/module shape for a new target — it tells you which files/classes should exist, includes conventional concrete companions for Interface files, and marks which inferred targets already exist but still differ; it does not write files for you`);
  lines.push(`10. Use \`cartograph_route_pairs\` when auditing nested endpoints against likely flat equivalents. It is best for route-shape inventory work like flat-vs-nested API audits`);
  lines.push(`11. Use \`cartograph_find\` before grepping — it searches the pre-built symbol index and suggests close matches on 0 results`);
  lines.push(`12. Use \`cartograph_search_content\` when you need method-body or literal-content lookup (for example \`$this->args['recurringJobID']\`)`);
  lines.push(`13. Use \`cartograph_symbol\` with \`deep: true\` to see the full vertical stack (Route→Controller→Builder→Model) in one call, including stack context requirements`);
  lines.push(`14. Use \`cartograph_compare\` for pairwise pattern-copy work — shows what A has that B doesn't, with method bodies and wiring targets inlined`);
  lines.push(`15. Use \`cartograph_compare_many\` when comparing one reference implementation against several siblings`);
  lines.push(`16. Use \`cartograph_deps\` to trace forward dependencies — shows which method creates each edge (e.g. "via getControllerName(), line 13")`);
  lines.push(`17. Use \`cartograph_flow\` to trace execution flow — follows parent class template methods through child overrides and surfaces thrown/caught exceptions along the traced stack\n`);
  ...
  lines.push('| `cartograph_sql_validate` | Validate literal SQL-ish table, column, and join refs in a symbol or file against the current indexed schema. Best for review questions like "does this Builder SQL match the schema?". Heuristic only: it checks literal refs and join predicates, not full runtime SQL semantics. | Exactly one of `symbol` or `file`. `limit`: max rows per section |');
  ...
  lines.push('| `cartograph_flow` | Trace execution flow from an entrypoint. Follows calls, `::class` refs, and parent class template methods (resolves `$this->getX()` through child overrides), and surfaces thrown/caught exceptions found on traced classes. | `depth`: 1-15 (default 5) |');
  ...
  lines.push(`- \`cartograph_sql_validate\` is heuristic. It validates literal SQL-ish refs and join predicates against the current schema, but it is not a full SQL parser and cannot prove runtime query correctness.`);
