...
export function generateClaudeMdSection(stats: RepoStats, conventions: ConventionsData): string {
  ...
  lines.push(`7. Use \`cartograph_scaffold_plan\` when cloning an existing endpoint/module shape for a new target — it tells you which files/classes should exist and which targets already exist but still differ`);
  ...
  lines.push('| `cartograph_scaffold_plan` | Plan the files and class names needed to mirror a reference slice for a new target stem. Marks which inferred targets already exist and summarizes their gaps vs the reference. | `reference`: reference symbol. `target`: target stem or class name. `depth`: 1-6 |');
